...
| E003 | 2026-01-26 | Delta SNN Training | ❌ Failed | Instable (RMSE > 200A) |
| E004 | — | Membrane SNN Full Training | 🔲 TODO | — |
| E005 | — | Hyperparameter: hidden_size | 🔲 TODO | — |
...
hidden_size: 64
num_hidden_layers: 2
delta_scale: 0.01  # Max change per spike ~1% of V_max (~3V)
delta_beta: 0.8
...
files_used: 276 (partial generation)
```

### Command
```bash
poetry run python -m evaluation.snn.train --model_type delta --epochs 50 --data_dir data/raw/train --device cuda
```

### Results

**Training Performance:**
- Final Train MAE: 0.0156 (normalized)
- Final Val MAE: 0.0156 (normalized)
- *Note: Training converged well on static imitation task.*

**Closed-Loop Benchmark (vs PI Baseline):**

| Metric | PI Baseline | Delta SNN (1 step) | Delta SNN (16 steps) |
|--------|-------------|--------------------|-----------------------|
| RMSE i_q | 78 mA | 207,936 mA | 228,622 mA |
| Stability | ✅ Stable | ❌ Unstable | ❌ Unstable |
| Overshoot | 73% | 4387% | 4715% |
| Latency | — | 2.0 ms | 35.9 ms |

### Insights & Failure Analysis

1.  **Slew Rate Limitation (The Main Culprit):**
    - The Delta SNN outputs *changes* in voltage ($\Delta u$).
    - With `delta_scale=0.01` and `u_max=300V`, each spike changes output by max **3V**.
    - **Problem:** At 1000 RPM, the motor's Back-EMF requires the voltage to jump significantly (e.g., 0V → 50V) very quickly during transients.
    - **Result:** The SNN physically cannot change the voltage fast enough to track the motor dynamics. It "falls behind," error accumulates, and the system destabilizes.

2.  **Inference Steps Didn't Help:**
    - We increased `inference_steps` to 16, theoretically allowing $16 \times 3V = 48V$ change per control step.
    - **Result:** Still unstable. The latency increased to **35ms** (too slow for 10kHz control loop which requires <0.1ms), exacerbating the instability.

3.  **Imitation vs. Control:**
    - The SNN learned to imitate the PI controller well on *static data* (low MAE during training).
    - However, in *closed-loop*, small errors accumulate. Because the Delta SNN integrates its own output (it remembers previous voltage), it acts like an open-loop integrator without sufficient feedback correction speed.

### Conclusion for Thesis
- **Delta/Incremental coding is poorly suited for PMSM control** where large, rapid voltage jumps are required (high bandwidth).
- **Absolute coding (Membrane Potential)** is likely superior because it can output any voltage value in a single timestep, independent of the previous value.
- **Future Direction:** Abandon Delta coding for this task; focus on **Membrane Potential** (Direct Voltage) readout.
...
### Configuration
```yaml
model_type: membrane  # Direct voltage readout
beta_output: 0.995    # Slow leak integration
# ... rest same as E003
```
