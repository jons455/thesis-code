...
Usage:
    python -m benchmark.compare_controllers
    
    # With custom checkpoint
    python -m benchmark.compare_controllers --checkpoint snn/checkpoints/best_model.pt
"""
...
try:
    from benchmark.agents import PIControllerAgent, SNNControllerAgent
    from benchmark.pmsm_env import PMSMEnv
except ImportError:
    from agents import PIControllerAgent, SNNControllerAgent
...
def plot_comparison(pi_data, snn_data, save_path=None):
    ...
    # i_q tracking
    ax = axes[0, 0]
    ax.plot(time_pi, [d['i_q_ref'] for d in pi_ep], 'k--', label='Reference', linewidth=2)
    ax.plot(time_pi, [d['i_q'] for d in pi_ep], 'b-', label='PI', linewidth=1.5)
    ax.plot(time_snn, [d['i_q'] for d in snn_ep], 'r-', label='SNN', linewidth=1.5, alpha=0.8)
    ...
    # i_d tracking
    ax = axes[0, 1]
    ax.plot(time_pi, [d['i_d_ref'] for d in pi_ep], 'k--', label='Reference', linewidth=2)
    ax.plot(time_pi, [d['i_d'] for d in pi_ep], 'b-', label='PI', linewidth=1.5)
    ax.plot(time_snn, [d['i_d'] for d in snn_ep], 'r-', label='SNN', linewidth=1.5, alpha=0.8)
    ax.set_xlabel('Time [ms]')
    ax.set_ylabel('i_d [A]')
    ...
    snn_error = np.sqrt(np.array([d['e_d']**2 + d['e_q']**2 for d in snn_ep])) * 1000
    ...
    ax.plot(time_snn, [d['u_q'] for d in snn_ep], 'r-', label='SNN u_q', linewidth=1.5, alpha=0.8)
    ax.set_xlabel('Time [ms]')
    ax.set_ylabel('u_q [V]')
    ...
    if snn_data['sparsities']:
        ...
        for key in snn_data['sparsities'][0].keys():
...
def print_metrics_table(pi_metrics, snn_metrics, snn_sparsities=None):
    ...
    print(f"{'total_variation':<25} {pi_tv:>12.2f} V  {snn_tv:>12.2f} V  {tv_ratio:>8.1f}x")
    ...
    if not np.isnan(pi_st) and not np.isnan(snn_st):
        ...
        print(f"{'settling_time':<25} {pi_st:>12.2f} ms {snn_st:>12.2f} ms {st_ratio:>8.1f}x")
    ...
    if snn_sparsities:
        ...
        avg_sparsity = {}
        for key in snn_sparsities[0].keys():
...
def main():
    ...
    parser.add_argument('--checkpoint', type=str, default='snn/checkpoints/best_model.pt',
                        help='Path to SNN checkpoint')
    parser.add_argument('--n_rpm', type=float, default=1000, help='Motor speed [rpm]')
    parser.add_argument('--i_q_ref', type=float, default=2.0, help='q-axis current reference [A]')
    ...
    if not checkpoint_path.exists():
        ...
        print("Run training first: python -m snn.train --epochs 100")
        return
    ...
    # Generate plots
    output_path = args.output or f"benchmark/comparison_{datetime.now().strftime('%Y%m%d_%H%M%S')}.png"
    plot_comparison(pi_data, snn_data, save_path=output_path)
