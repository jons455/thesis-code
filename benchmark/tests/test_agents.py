...
from agents import PIControllerAgent, PIControllerTorchAgent, SNNControllerAgent, SNNControllerTorchAgent
...
class TestSNNControllerAgent:
    ...
    def checkpoint_path(self):
        ...
        return Path(__file__).parent.parent.parent / "snn" / "checkpoints" / "best_model.pt"
    ...
    def test_call_returns_action(self, checkpoint_path):
        ...
        agent = SNNControllerAgent(str(checkpoint_path))
        state = np.array([0.0, 0.0, 0.1, 0.2])  # i_d, i_q, e_d, e_q
        action = agent(state)
    ...
    def test_state_persists_across_calls(self, checkpoint_path):
        ...
        # Second call uses persisted state
        action2 = agent(state)
        state_after_2 = agent._snn_state
        
        # State should be different (membrane potentials changed)
        # Can't compare directly due to tuple of tensors, but should not be None
    ...
    def test_get_sparsity(self, checkpoint_path):
        ...
        # Sparsity values should be between 0 and 1
    ...
    def test_no_nan_output(self, checkpoint_path):
        ...
        for _ in range(100):
            state = np.random.randn(4) * 0.5  # Random normalized states
            action = agent(state)
...
class TestSNNControllerTorchAgent:
    ...
    def test_forward_accepts_tensor(self, checkpoint_path):
        """Forward accepts PyTorch tensor input."""
        if not checkpoint_path.exists():
            pytest.skip("No SNN checkpoint available")
        
        agent = SNNControllerTorchAgent(str(checkpoint_path))
        state = torch.tensor([[0.0, 0.0, 0.1, 0.2]])
        action = agent(state)
        
        assert isinstance(action, torch.Tensor)
    
    def test_forward_returns_correct_shape(self, checkpoint_path):
        """Forward returns action with correct shape."""
        if not checkpoint_path.exists():
            pytest.skip("No SNN checkpoint available")
    ...
    def test_has_reset_method(self, checkpoint_path):
        """Agent has reset method."""
        if not checkpoint_path.exists():
            pytest.skip("No SNN checkpoint available")
