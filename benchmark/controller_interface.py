...
to be tested with the PMSM current control benchmark.

The interface is designed to be NeuroBench-compatible while remaining simple
enough for any control approach (classical, NN, SNN, hybrid).

Usage
-----
To test your controller with this benchmark:

1. Implement the ControllerInterface protocol
2. Wrap your controller in a ControllerAgent subclass
3. Run the benchmark:
...
----------------------------------
    class MyController(ControllerAgent):
        ...
        def __call__(self, state: np.ndarray) -> np.ndarray:
            # state = [i_d, i_q, e_d, e_q] normalized to [-1, 1]
            action = self.my_model.predict(state)
            return np.clip(action, -1.0, 1.0)  # [u_d, u_q] normalized
...
class ControllerInterface(Protocol):
    ...
    This uses Python's Protocol for structural typing - any class that
    implements these methods is automatically compatible, no inheritance needed.
    """
    ...
    def __call__(self, state: np.ndarray) -> np.ndarray:
        ...
        ----------
        state : np.ndarray
            Normalized state vector [i_d, i_q, e_d, e_q] in range [-1, 1]:
            - i_d: d-axis current / i_max
            - i_q: q-axis current / i_max
            - e_d: d-axis error (i_d_ref - i_d) / i_max
            - e_q: q-axis error (i_q_ref - i_q) / i_max
        
        Returns
        -------
        np.ndarray
            Normalized action [u_d, u_q] in range [-1, 1]:
            - u_d: d-axis voltage / u_max
            - u_q: q-axis voltage / u_max
        """
        ...
...
class ControllerAgent(ABC):
    ...
    This follows the NeuroBench agent interface pattern.
    """
    ...
    def get_neuromorphic_metrics(self) -> Optional[Dict[str, Any]]:
        """
...
class BenchmarkResults:
    ...
    spikes_per_step: Optional[float] = None
    sparsity: Optional[float] = None
    num_neurons: Optional[int] = None
    num_synapses: Optional[int] = None
    inference_latency_mean: Optional[float] = None
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        return {
            ...
            } if self.total_spikes is not None else None,
        }
    ...
    def summary(self) -> str:
        ...
        if self.total_spikes is not None:
            lines.extend([
                ...
                f"  Latency: {self.inference_latency_mean*1e6:.1f} µs" if self.inference_latency_mean else "",
            ])
...
def run_benchmark(
    ...
    if not isinstance(controller, ControllerInterface):
        raise TypeError(
            f"Controller must implement ControllerInterface: "
            f"__call__(state) -> action and reset()"
        )
    ...
    results = BenchmarkResults(
        ...
        inference_latency_mean=neuro_metrics.get('inference_latency_mean') if neuro_metrics else None,
    )
...
def _compute_settling_time(time: np.ndarray, y: np.ndarray, ref: np.ndarray, 
...
def _compute_rise_time(time: np.ndarray, y: np.ndarray, ref: np.ndarray,
    ...
    for i in range(1, len(y)):
        if t_lower is None:
            if (y[i-1] <= lower_val <= y[i]) or (y[i-1] >= lower_val >= y[i]):
                t_lower = time[i]
        if t_upper is None:
            if (y[i-1] <= upper_val <= y[i]) or (y[i-1] >= upper_val >= y[i]):
                t_upper = time[i]
...
def compare_controllers(controllers: list, env, config: Optional[BenchmarkConfig] = None) -> dict:
    """
    ...
    print("=" * 70)
    print(f"{'Controller':<20} {'RMSE_iq [mA]':>12} {'Settling [ms]':>14} {'Overshoot [%]':>14}")
    print("-" * 70)
    
    for name, result in results.items():
        print(f"{name:<20} {result.rmse_iq*1000:>12.2f} {result.settling_time_iq*1000:>14.1f} {result.overshoot_iq:>14.1f}")
