...
def validate_episode(df: pd.DataFrame, threshold: float = 0.5) -> bool:
    ...
    final_error_d = abs(df['i_d'].iloc[-1] - df['i_d_ref'].iloc[-1])
...
def main():
    ...
    parser.add_argument('--output-dir', type=str, default='pmsm-pem/export/train_v2',
                       help='Output directory')
    parser.add_argument('--max-steps', type=int, default=2000,
                       help='Steps per episode')
    parser.add_argument('--validate', action='store_true',
                       help='Only keep files with good tracking')
    ...
    for i in tqdm(range(args.num_files), desc="Generating"):
