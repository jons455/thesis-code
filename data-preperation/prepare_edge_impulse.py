...
def downsample_data(df, factor=10, preserve_time=True):
    ...
    for run_id in sorted(df["run_id"].unique()):
        ...
        if not preserve_time:
            ...
            original_dt = (
                run_data["time"].iloc[1] - run_data["time"].iloc[0] if len(run_data) > 1 else 0.0001
            )
...
def save_datasets(
    df_train, df_val, df_test, output_dir="edge_impulse_data", prefix="", add_timestamp=True
):
    """
...
def main():
    ...
    if user_input == "y":
        ...
        save_datasets(
            df_train_w, df_val_w, df_test_w, prefix=f"windowed{window_size}_{rate_label}_"
        )
