...
def get_sources(n_rpm: int) -> dict:
    return {
        ...
        / "export"
        / "validation"
        / f"validation_sim_n{n_rpm:04d}.csv",
        "GEM Standard": PMSM_PEM_DIR
        / "export"
        / "gem_standard"
        / f"sim_n{n_rpm:04d}.csv",
...
def plot_all_signals(dataframes: dict, output_path: Path, n_rpm: int):
    ...
    fig.suptitle(
        f"PMSM Simulation - Dreier-Vergleich @ {n_rpm} rpm",
        fontsize=14,
        fontweight="bold",
    )
    plt.tight_layout()
...
def plot_errors_vs_matlab(dataframes: dict, output_path: Path, n_rpm: int):
    ...
    axes[-1].set_xlabel("Zeit [ms]")
    fig.suptitle(
        f"Fehler relativ zu MATLAB @ {n_rpm} rpm", fontsize=14, fontweight="bold"
    )
    plt.tight_layout()
    plt.savefig(output_path, dpi=150, bbox_inches="tight")
    plt.close()
...
def plot_summary(all_results: list, output_path: Path):
    ...
    plt.suptitle(
        "Vergleich über alle Drehzahlen (Steady-State)", fontsize=14, fontweight="bold"
    )
    plt.tight_layout()
    plt.savefig(output_path, dpi=150, bbox_inches="tight")
    plt.close()
        ...
        for window_name, t0, t1 in windows:
            ...
            for signal in SIGNALS:
                if signal in df_test_w.columns and signal in df_ref_w.columns:
                    m = compute_metrics(
                        df_ref_w[signal].values, df_test_w[signal].values
                    )
                    results.append(
                        {
                            "n_rpm": n_rpm,
            ...
            if window_name == "steady_state":
                for r in results:
                    if (
                        ...
                        print(
                            f"    {window_name} i_q: MAE={r['MAE']:.4f}, RMSE={r['RMSE']:.4f}"
                        )
                        break
