...
def get_steady_state_value(
    df: pd.DataFrame, signal: str, t_start: float = 0.1
) -> float:
...
def plot_operating_point_comparison(df_results: pd.DataFrame, output_dir: Path):
    ...
    if "id_error_gem" in df_results.columns:
        ax.bar(
            x - width / 2,
            df_results["id_error_gem"],
            width,
            label="GEM Standard",
            color=COLORS["GEM Standard"],
        )
    if "id_error_own" in df_results.columns:
        ax.bar(
            x + width / 2,
            df_results["id_error_own"],
            width,
            label="Eigener Ctrl",
            color=COLORS["GEM Eigener Ctrl"],
        )
    ...
    ax.set_xticks(x)
    ax.set_xticklabels(
        [f"id={r.id_ref:+.0f}\niq={r.iq_ref:.0f}" for r in df_results.itertuples()],
        fontsize=8,
    )
    ax.legend()
    ...
    plt.suptitle(
        f"Arbeitspunkt-Vergleich @ {N_RPM} rpm\nController-Vergleich",
        fontsize=14,
        fontweight="bold",
    )
    plt.tight_layout()
...
def plot_time_series_all_ops(output_dir: Path):
    ...
    for idx, (id_val, iq_val, name) in enumerate(OPERATING_POINTS):
        ...
        # Sollwert-Linien
        ax_id.axhline(
            id_val, color="red", linestyle=":", linewidth=1, label=f"Soll: {id_val} A"
        )
        ax_iq.axhline(
            iq_val, color="red", linestyle=":", linewidth=1, label=f"Soll: {iq_val} A"
        )
    ...
    plt.suptitle(
        f"Zeitverläufe aller Arbeitspunkte @ {N_RPM} rpm",
        fontsize=14,
        fontweight="bold",
    )
    plt.tight_layout()
...
def main():
    ...
    if not df_results.empty:
        ...
        for _, row in df_results.iterrows():
            ...
            print(
                f"  id={row['id_ref']:+.0f} A, iq={row['iq_ref']:.0f} A: "
