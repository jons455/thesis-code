...
def parse_args():
    p = argparse.ArgumentParser(
        description="GEM PMSM current-control simulation (standard GEM controller)"
    )
    p.add_argument(
        "--n-rpm",
        type=float,
    ...
    p.add_argument(
        ...
        type=float,
        default=0.0,
        help="Step time in seconds (0 = immediate).",
    )
    p.add_argument(
...
for i in range(number_of_simulations):
    ...
    for k in range(args.sim_steps):
        ...
        omega_mech = state[idx_omega] * actual_limits.get(
    ...
    filename = os.path.join(
        out_dir, args.output if args.output else f"sim_{i+1:04d}.csv"
    )
