...
from embark.utils.config import DEFAULT_MAX_STEPS, DEFAULT_PMSM  # noqa: E402
...
motor_parameter = {
    "p": DEFAULT_PMSM.p,
    "r_s": DEFAULT_PMSM.r_s,
    "l_d": DEFAULT_PMSM.l_d,
    "l_q": DEFAULT_PMSM.l_q,
    "psi_p": DEFAULT_PMSM.psi_p,
}

limit_values = {
    "i": DEFAULT_PMSM.i_max,
    "u": DEFAULT_PMSM.u_max,
    "omega": DEFAULT_PMSM.omega_max,
}

# Simulationsparameter
tau = DEFAULT_PMSM.tau
sim_steps = DEFAULT_MAX_STEPS
number_of_simulations = 1


def parse_args():
    p = argparse.ArgumentParser(
        description="GEM PMSM current-control simulation (MATLAB-matching PI controller)"
    )
    p.add_argument(
        "--n-rpm",
        type=float,
        default=716.0,
        help="Fixed mechanical speed in rpm (ConstantSpeedLoad).",
    )
    p.add_argument(
        "--id-ref",
        type=float,
        default=0.0,
        help="d-axis current reference after step [A].",
    )
    p.add_argument(
        "--iq-ref",
        type=float,
        default=3.5,
        help="q-axis current reference after step [A]. Run 003: 3.5A",
    )
    p.add_argument("--step-time", type=float, default=0.1, help="Step time in seconds.")
    p.add_argument(
        "--sim-steps", type=int, default=sim_steps, help="Number of simulation steps."
    )
    p.add_argument(
        "--tau", type=float, default=tau, help="Control cycle time in seconds."
    )
    p.add_argument(
        "--output",
        type=str,
        default=None,
        help="Output filename (default: sim_0001.csv)",
    )
    return p.parse_args()
...
print(
    f"GEM Limits: omega={actual_limits.get('omega', 0)*60/(2*np.pi):.0f} rpm, "
...
controller = PIController(
    ...
    ki_q=K_Iq,
    ts=args.tau,
    u_max=limit_values["u"],
    l_d=motor_parameter["l_d"],
    l_q=motor_parameter["l_q"],
    psi_pm=motor_parameter["psi_p"],
    p=motor_parameter["p"],
)
...
def get_state_indices(env, state_vec):
    try:
        if hasattr(env, "state_names"):
            return (
                env.state_names.index("i_sd"),
                env.state_names.index("i_sq"),
                env.state_names.index("omega"),
                env.state_names.index("epsilon"),
            )
...
for i in range(number_of_simulations):
    ...
    for k in range(args.sim_steps):
        ...
        omega_mech = state_vec[idx_omega] * actual_limits.get(
            "omega", limit_values["omega"]
        )
        ...
        u_d_norm, u_q_norm = controller.control(
            i_d, i_q, id_ref_active, iq_ref_active, omega_elec
        )
