...
def run_simulation(
    ...
    print(
        f"  [{controller_type}] id={id_ref:+.1f} A, iq={iq_ref:+.1f} A -> {output_name}"
    )

    try:
        result = subprocess.run(
            cmd, capture_output=True, text=True, cwd=str(SCRIPT_DIR), check=False
        )
...
def main():
    ...
    for id_val, iq_val, name in OPERATING_POINTS:
        ...
        print(
            f"  {valid} {name}: id={id_val:+.1f} A, iq={iq_val:+.1f} A, |I|={i_total:.2f} A"
        )
    ...
    for id_val, iq_val, _name in OPERATING_POINTS:
