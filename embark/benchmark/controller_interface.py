...
from embark.benchmark.processors import ProcessorBundle, get_default_processors
...
class ControllerInterface(Protocol):
    ...
    def __call__(self, state: np.ndarray) -> np.ndarray:
        ...
        Args:
            state: State vector [i_d, i_q, e_d, e_q]. By default this is
                normalized to [-1, 1], but controllers can declare
                INPUT_SPACE/OUTPUT_SPACE to receive physical units.

        Returns:
            Action [u_d, u_q]. By default this is normalized to [-1, 1].
        """
        ...
...
class ControllerAgent(ABC):
    ...
    def __call__(self, state: np.ndarray) -> np.ndarray:
        ...
        ----------
        state : np.ndarray
            State [i_d, i_q, e_d, e_q], typically normalized to [-1, 1]

        Returns
        -------
        np.ndarray
            Action [u_d, u_q], typically normalized to [-1, 1]
        """
        pass
...
class BenchmarkConfig:
    ...
    # Simulation
    max_steps: int = DEFAULT_MAX_STEPS  # Maximum steps per episode
    ...
    tau: float = DEFAULT_PMSM.tau  # Control period [s] (10 kHz)
...
def run_benchmark(
    ...
    verbose: bool = True,
    processors: ProcessorBundle | None = None,
) -> BenchmarkResults:
    """
    ...
    if processors is None:
        processors = get_default_processors(controller, config.i_max, config.u_max)
    ...
    for _ in range(config.max_steps):
        ...
        u_d_list.append(float(action_env[0]))
        u_q_list.append(float(action_env[1]))
