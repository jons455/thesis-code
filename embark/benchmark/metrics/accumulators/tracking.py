...
class MaximumError(MetricAccumulator):
    ...
    tracked_keys: list[str]
    _max_errors: dict[str, float] | None = None
    ...
    def update(
        ...
        for key in self.tracked_keys:
            ref_key = f"{key}_ref"
            error = abs(reference[ref_key] - state[key])
            if error > self._max_errors[key]:
                self._max_errors[key] = float(error)

    def compute(self) -> dict[str, float]:
        return {f"max_error_{key}": self._max_errors[key] for key in self.tracked_keys}
