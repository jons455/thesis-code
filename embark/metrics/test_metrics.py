...
from embark.metrics import (  # noqa: E402
...
def test_with_synthetic_data():
    ...
    response = np.where(
        ...
        * (
            1
            - np.exp(-zeta * omega_n * t_from_step)
            * (
    ...
    efficiency = compute_efficiency_metrics(time, i_d, i_q, u_d, u_q, speed)
...
def test_neuromorphic_metrics():
    ...
    from embark.metrics import compute_neuromorphic_metrics_from_spikes
    ...
    spike_trains = (
        np.random.random((num_neurons, num_timesteps)) < spike_probability
    ).astype(float)
