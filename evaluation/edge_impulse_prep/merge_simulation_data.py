...
def merge_panel(files, time_col: str = "time") -> pd.DataFrame:
    ...
    panel = panel.sort_values(["run_id", time_col], kind="mergesort").reset_index(
        drop=True
    )
    return panel
...
def main(
    ...
    if not files:
        raise SystemExit(
            f"No files found in {input_dir!r} matching pattern {pattern!r}"
        )
...
if __name__ == "__main__":
    ...
    p.add_argument(
        ...
        type=str,
        default="sim_*.csv",
        help="Glob pattern (default: sim_*.csv)",
    )
    ...
    p.add_argument(
        ...
        type=str,
        default=None,
        help="Output directory (default: data/processed/merged)",
    )
    p.add_argument(
        "--mode",
        type=str,
        choices=["panel", "stack", "both"],
        default="both",
        help="Output mode",
    )
    args = p.parse_args()
