...
from embark.utils.paths import DATA_PROCESSED_DIR  # noqa: E402


def load_data(use_parquet=True):
    """Load the merged panel data."""
    data_dir = DATA_PROCESSED_DIR / "merged"
...
def downsample_data(df, factor=10, preserve_time=True):
    ...
    for runs_processed, run_id in enumerate(run_ids, start=1):
        ...
        if runs_processed % 100 == 0:
            print(f"  Processed {runs_processed}/{len(run_ids)} runs...")
    ...
    print(
        f"Reduction: {original_samples/new_samples:.1f}× ({original_samples-new_samples:,} samples removed)"
    )
...
def save_datasets(
    df_train, df_val, df_test, output_dir=None, prefix="", add_timestamp=True
):
    """
    ...
    if add_timestamp:
        ...
        df_train_copy.insert(
            0, "timestamp", np.arange(len(df_train_copy)) * 1.0
        )  # 1 ms steps
    ...
    print(
        f"  ✓ Train: {train_file.name} ({train_file.stat().st_size / 1024 / 1024:.1f} MB)"
    )
    print(
        f"  ✓ Val:   {val_file.name} ({val_file.stat().st_size / 1024 / 1024:.1f} MB)"
    )
    print(
        f"  ✓ Test:  {test_file.name} ({test_file.stat().st_size / 1024 / 1024:.1f} MB)"
    )
...
def print_data_info(df_train, df_val, df_test):
    ...
    for col in df_train.columns:
        ...
        print(
            f"{col:8s}: Train [{train_range[0]:7.3f}, {train_range[1]:7.3f}] | "
...
def main():
    ...
    if downsample_input == "" or downsample_input == "1":
        ...
        try:
            ...
            rate_label = (
                f"{10/downsample_factor:.0f}hz" if downsample_factor > 1 else "10khz"
            )
        except ValueError:
    ...
    if user_input == "y":
        ...
        save_datasets(
            ...
            df_val_w,
            df_test_w,
            prefix=f"windowed{window_size}_{rate_label}_",
        )
