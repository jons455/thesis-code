...
poetry run python evaluation/run_evaluation.py --save-results evaluation/results/run_001.json
```
...
| `models/best_model.pt` | Pre-trained SNN (imitation learning from PI) |
...
controller = SNNBenchmarkController(
    "evaluation/models/best_model.pt",
    device="cpu",
    track_spikes=True,
)
...
for step in range(2000):
    action = controller(state)  # [u_d, u_q] normalized
