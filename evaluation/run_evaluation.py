...
Usage:
    ...
    # With custom options
    poetry run python evaluation/run_evaluation.py --speed 1500 --iq-ref 3.0
"""
...
from benchmark import PIControllerAgent, PMSMEnv
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--model",
        type=str,
        default="evaluation/models/best_model.pt",
...
def main() -> int:
    ...
    if not model_path.exists():
        ...
        print("Please ensure the model file exists or specify a valid path with --model")
        return 1
    ...
    print(f"  PI Controller: Ready")
    snn_info = snn_controller.get_info()
    print(f"  SNN Controller: {snn_info['parameters']} parameters, "
    ...
    # Accuracy metrics
    print(f"{'RMSE i_q [mA]':<25} {pi_results.rmse_iq*1000:>18.2f} "
          f"{snn_results.rmse_iq*1000:>18.2f}")
    print(f"{'RMSE i_d [mA]':<25} {pi_results.rmse_id*1000:>18.2f} "
          f"{snn_results.rmse_id*1000:>18.2f}")
    print(f"{'MAE i_q [mA]':<25} {pi_results.mae_iq*1000:>18.2f} "
          f"{snn_results.mae_iq*1000:>18.2f}")

    # Dynamics metrics
    print(f"{'Settling time i_q [ms]':<25} {pi_results.settling_time_iq*1000:>18.1f} "
          f"{snn_results.settling_time_iq*1000:>18.1f}")
    print(f"{'Rise time i_q [ms]':<25} {pi_results.rise_time_iq*1000:>18.1f} "
          f"{snn_results.rise_time_iq*1000:>18.1f}")
    print(f"{'Overshoot i_q [%]':<25} {pi_results.overshoot_iq:>18.1f} "
    ...
    if snn_results.total_spikes is not None:
        ...
        print(f"{'Spikes per step':<25} {'-':>18} "
              f"{snn_results.spikes_per_step:>18.1f}")
        print(f"{'Sparsity [%]':<25} {'-':>18} "
              f"{snn_results.sparsity*100:>17.1f}%")
        ...
        if snn_results.inference_latency_mean:
            print(f"{'Inference latency [µs]':<25} {'-':>18} "
