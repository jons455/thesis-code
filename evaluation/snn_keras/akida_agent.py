...
standard ControllerInterface used in the embark benchmark suite.
"""
...
from embark.benchmark.controller_interface import ControllerAgent
from embark.utils.config import DEFAULT_PMSM
from evaluation.snn_keras.models import AkidaController, AkidaConfig


class AkidaControllerAgent(ControllerAgent):
    """
    ...
    def __call__(self, state: np.ndarray) -> np.ndarray:
        ...
        Args:
            state: Physical state [i_d, i_q, e_d, e_q] (and optionally n)
        
        Returns:
            action: Physical action [u_d, u_q]
        """
        # Ensure flat array
        ...
        # Handle speed if present (Env returns 4 or 5 depending on config)
        # If not present, assume 0 or last known
        n_rpm = state[4] if len(state) > 4 else 0.0
        ...
        input_vector = np.array([[i_d_norm, i_q_norm, e_d_norm, e_q_norm, n_norm]], dtype=np.float32)
