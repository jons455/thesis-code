...
This module provides tools for exporting the trained SNN controller
to neuromorphic hardware formats, particularly:
- NIR (Neuromorphic Intermediate Representation) - universal format
- Akida-compatible format via CNN2SNN
...
Usage:
    ...
    # Check compatibility
    issues = validate_akida_compatibility(model)
...
def validate_akida_compatibility(model: nn.Module) -> AkidaCompatibilityReport:
    ...
    Akida constraints:
    - Only specific layer types (Dense, Conv2D, etc.)
    - LIF neurons with specific parameters
    ...
    for name, module in model.named_modules():
        ...
        if hasattr(module, 'beta'):  # snnTorch neuron
            ...
            if hasattr(module, 'reset_mechanism'):
                if module.reset_mechanism == "none":
                    has_no_reset = True
                    warnings.append(
        ...
        if isinstance(module, nn.BatchNorm1d):
            warnings.append(f"BatchNorm '{name}' will be folded into weights during quantization")
    ...
    for name, param in model.named_parameters():
        if param.dtype != torch.float32:
            warnings.append(f"Parameter '{name}' is {param.dtype}, will be quantized to INT8")
    
    # Check for quantization
    recommendations.append("Quantize weights to INT8 before deployment (use quantization-aware training)")
...
def export_to_nir(
    ...
    NIR is a universal format that can be converted to:
    - Akida (via nir2akida, when available)
    - Loihi (via nir2loihi)
    - SpiNNaker (via nir2spinnaker)
    - Speck (via nir2speck)
    ...
    model : nn.Module
        Trained SNN model
    output_path : str
        Path to save the .nir file
    sample_input : torch.Tensor, optional
        Sample input for tracing. If None, uses default shape [1, 4].
    ...
    try:
        ...
        print(f"NIR export requires: pip install nir")
        print(f"snnTorch NIR support requires: pip install snntorch>=0.9.0")
        raise ImportError(
            "Install NIR support: pip install nir snntorch>=0.9.0"
        ) from e
    ...
    # Export to NIR
    print(f"Exporting model to NIR format...")
...
class RateCodingOutput(nn.Module):
    ...
    def forward(
        ...
        ----------
        x : torch.Tensor
            Input from previous layer [batch, features]
        mem : torch.Tensor, optional
            Previous membrane state
        
        Returns
        -------
        output : torch.Tensor
            Decoded continuous output [batch, output_size]
        mem : torch.Tensor
            Updated membrane state
        """
...
class PopulationCodingOutput(nn.Module):
    ...
    Multiple neurons per output dimension, each tuned to a different
    value. The population activity is decoded to a continuous value.
    ...
    def forward(
        ...
        if mem is None:
            mem = torch.zeros(
                batch_size, 
                self.output_size * self.neurons_per_output,
                device=device
            )
...
def quantize_model(
    ...
    calibration_data : torch.Tensor, optional
        Data for calibrating activation ranges
    bits : int
        Number of bits (default: 8 for Akida)
...
def main():
    ...
    parser.add_argument(
        "--checkpoint", type=str, default="snn/checkpoints/best_model.pt",
        help="Path to model checkpoint"
    )
    parser.add_argument(
        "--output", type=str, default="exports/pmsm_controller.nir",
    ...
    parser.add_argument(
        "--format", type=str, choices=["nir", "onnx"], default="nir",
        help="Export format"
    )
