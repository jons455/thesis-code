...
The dataset provides (input, target) pairs where:
- Input: [i_d, i_q, e_d, e_q] normalized state
- Target: [u_d, u_q] normalized voltage command

Data is loaded from CSV files generated by the PI controller simulations.
...
class PMSMDataset(Dataset):
    ...
    Loads CSV files from a directory and extracts windows of:
    - Input: [i_d, i_q, e_d, e_q] (currents and errors)
    - Target: [u_d, u_q] (voltage commands from PI controller)
    ...
    data_dir : str or Path
        Directory containing CSV trajectory files
    config : DataConfig, optional
        Data loading configuration
    window_size : int, optional
        Override config window size
    stride : int, optional
        Override config stride
    file_pattern : str
        Glob pattern for CSV files (default: "*.csv")
    max_files : int, optional
        Maximum number of files to load (for debugging)
    
    Example
    -------
        # Load all trajectories
        dataset = PMSMDataset("pmsm-pem/export/train")
        
        # With custom window size
        dataset = PMSMDataset(
            "pmsm-pem/export/train",
            window_size=200,
            stride=100,
        )
    ...
    def _find_columns(self, df: pd.DataFrame) -> Tuple[List[str], List[str], List[str]]:
        ...
        for c in i_cols + u_cols:
            if c not in cols:
                raise ValueError(
                    f"Column '{c}' not found in data. "
                    f"Available: {cols}"
                )
        
        return i_cols, u_cols, ref_cols
    
    def _load_file(self, filepath: Path) -> Optional[Tuple[np.ndarray, np.ndarray]]:
        """
        Load a single CSV file and extract input/target arrays.
        
        Returns None if file is too short or has issues.
        """
        try:
