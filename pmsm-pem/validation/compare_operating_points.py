...
def get_steady_state_value(df: pd.DataFrame, signal: str, t_start: float = 0.1) -> float:
...
def plot_operating_point_comparison(df_results: pd.DataFrame, output_dir: Path):
    ...
    ax.set_xticks(x)
    ax.set_xticklabels(
        [f"id={r.id_ref:+.0f}\niq={r.iq_ref:.0f}" for r in df_results.itertuples()], fontsize=8
    )
    ax.legend()
...
def plot_time_series_all_ops(output_dir: Path):
    ...
    plt.suptitle(f"Zeitverläufe aller Arbeitspunkte @ {N_RPM} rpm", fontsize=14, fontweight="bold")
    plt.tight_layout()
